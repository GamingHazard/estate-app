...
export const PROPERTY_SPECIFICATIONS: Record<string, SpecificationTemplate[]> =
  {
    ...
    "commercial-land": [
      ...
      {
        key: "maxBuildout",
        label: "Max Buildout Potential",
        placeholder: "e.g. 150000 sq ft",
      },
    ],
  };
...
export function normalizeCategory(value: string): PropertyCategory {
  const normalized = value.trim().toLowerCase();
  if (normalized in PROPERTY_CATEGORIES) {
...
export function normalizePropertyType(
  ...
  const normalized = value.trim().toLowerCase();
  if (normalized in PROPERTY_CATEGORIES[category].types) {
    return normalized;
  }
  return Object.keys(PROPERTY_CATEGORIES[category].types)[0];
}
